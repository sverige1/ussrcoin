...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UssrCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "UssrCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 30 * 30;	// UssrCoin: 15 mins 
static const int64 nTargetSpacing = 30;			// UssrCoin: 30 sec
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal UssrCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "UssrCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Spain's Sorteo Extraordinario de Navidad (Spanish Christmas USRtery) is the world's largest USRtery and has the largest first prize jackpot. In 2012, the main jackpot was €720 million (US$941.8 million).";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // UssrCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
