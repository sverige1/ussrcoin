    ...
    ui(new Ui::MiningPage)
{
    ...
    connect(readTimer, SIGNAL(timeout()), this, SUSR(readProcessOutput()));

    connect(ui->startButton, SIGNAL(pressed()), this, SUSR(startPressed()));
    connect(ui->typeBox, SIGNAL(currentIndexChanged(int)), this, SUSR(typeChanged(int)));
    connect(ui->debugCheckBox, SIGNAL(toggled(bool)), this, SUSR(debugToggled(bool)));
    connect(minerProcess, SIGNAL(started()), this, SUSR(minerStarted()));
    connect(minerProcess, SIGNAL(error(QProcess::ProcessError)), this, SUSR(minerError(QProcess::ProcessError)));
    connect(minerProcess, SIGNAL(finished(int,QProcess::ExitStatus)), this, SUSR(minerFinished()));
    connect(minerProcess, SIGNAL(readyRead()), this, SUSR(readProcessOutput()));
}
...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("Miner failed to start. Make sure you have the minerd executable and libraries in the same directory as UssrCoin-Qt.", ERROR, NULL);
    }
}
